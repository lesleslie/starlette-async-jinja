...
async def test_error_handling(mock_template: MagicMock) -> None:
    ...
    # Create the mocks separately to avoid direct assignment
    mock_get_template_async = AsyncMock(
    ...
    mock_render_fragment_async = AsyncMock(side_effect=mock_render_fragment)

    # Apply the mocks using patch context manager for the rest of the function
    with (
        patch.object(templates, "get_template_async", mock_get_template_async),
        patch.object(templates, "render_fragment", mock_render_fragment_async),
    ):
